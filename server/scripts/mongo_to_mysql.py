...
def init():
    conn = MySQLdb.connect(host='localhost',user='crawler',passwd='crawlerpwd',db='xiaoshuo_test',port=3306, charset='utf8')
    cur = conn.cursor()
...
def chapter(conn, cur, mdb, tmpbid, curbid, create_time):
    sql = '''INSERT INTO chapter(
            book_id, title, content, create_time)
    	    VALUES (%s, %s, %s, %s);'''
	...
	ch['curbid'] = curbid
	    ...
	    #查询当前章节是否存在
	    num = cur.execute("select id from chapter where book_id=%s and title=%s", (curbid, ch['title']))
